...
import com.gtan.repox.HeadResultCache.{Query, NotFound}
...
class OldHeadWorker(val exchange: HttpServerExchange, val upstreams: List[Repo]) extends LazyLogging {
  ...
  def `try`(times: Int): Unit = {
    if (times == 0) {
      ...
      val filteredUpstreams: Future[List[Repo]] = (headResultCache ? Query(uri)).map {
